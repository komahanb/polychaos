...
14         continue
           close(10)
           !       write(filenum,*)st,ntgt,ftgt,NMCS
...
!!$           write(filenum,'(6x,a,e15.5)')'>> Standard Deviation = ',st
!!$           call CDF(          -1.d0*dx,0.d0,st,p1)
!!$           call CDF_Numerical(-1.d0*dx,0.d0,st,p2)
!!$           write(filenum,'(6x,a,2e15.5,a,f12.7)') &
...
!!$              if(ntgt.eq.0)then
!!$                 call lhs_normal_dist(ndim,NMCS,xavg,st,MNCx)
!!$              else
!!$                 call lhs_normal_dist(1,NMCS,xavg(ntgt),st,MNCx)
!!$              end if

           end if

           ! Map samples to Kriging space
           do j=1,NMCS
              do k=1,ndim
...
!!$           end if
           ...
           if (hstat.eq.0) write(94,'(i4,6e16.8)') nhs,Javg(1),Jvar(1),fmean,fvar,abs(fmean-Javg(1)),abs(fvar-Jvar(1))
...
! Maybe accurate within the range of [-4:4] with n=100
        ...
        if(yout.gt.1.d0)write(filenum,'(a,2e15.5)')'*ERF>1 ',xin,yout-1.d0

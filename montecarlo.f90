subroutine montecarlo(stat,fct,NDIM,dimpc,nterms,npts,ipar,xcof)
  ...
  read(10,*) !(xavg(i),i=1,ndim) == aleady reading or setting in main.f90
  read(10,*) !(xstd(i),i=1,ndim) == aleady reading or setting in main.f90    
  read(10,*)
  read(10,*)
  read(10,*) NMCS
  read(10,*) npdf
  ...
  do i=is,ie ! Main Loop for MC
     ...
     x(:)    = MNCx(:,i)                           ! store current sample as x

     do j=1,ndim
        xmin(j) = min(xmin(j),x(j)) ! find min x value
        xmax(j) = max(xmax(j),x(j)) ! find max x value
     end do
     ...
     !!  call evalcostf(stat,ndim,fct,x,fv,gv,hv,DS)  !Evaluate exact function
     !                print *, 'x:',x,'PC:',yhat!,'Ex:',fv ,id_proc

     ict    = ict + 1       ! add 1 to counter
     ...
     MNCf(i) = yhat ! response value at x (mean)

     if(yhat.ge.ymax)then ! find max response value
        ymax = yhat
     end if

     if(yhat.le.ymin)then ! find min response value
        ymin = yhat
     end if
  ...
  do id=0,num_proc-1
     ...
     ! Exchange response values for NMCS samples
  ...
  ! share max and min along each x dimension
  ...
  ! share mean and variance values
  ...
  ! share derivatives of mean and variance
  do i=1,ndim
  ...
  ! share Hessian of mean and variance
  ...
  !=======================================================================!
  !================find mean, var, their derivs and hess =================!
  !=======================================================================!
...
!!$              hglb=0.0d0 !remove it
        ...
        if(id_proc.eq.0)then
           ...
           fstd=sqrt(fvar)
           fmeanprime(1:ndim) = fmeanprime(1:ndim)
           fvarprime= fvarprime(1:ndim)

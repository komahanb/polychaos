...
!  ------------------------------------------------------------------------------
!
  subroutine TimerReport(ttotal)
    implicit none
    
    integer :: i,sl,count,count_rate,count_max
    real,intent(out) :: ttotal
...
!!$    endif
    ...
    do i=1,NCurTimer
      !sl = SortIndex( i )
      !write(filenum,10) TimerName(sl),TotalTime(sl),PercentTime(sl)
       write(filenum,10) TimerName(i),TotalTime(i),PercentTime(i) 
    enddo
    print *,'---------------------------------------------------------'
    write(filenum,20) ttotal
    print *,'---------------------------------------------------------'
